...
export function CustomCursor({
  ...
  // Custom cursors hide the native pointer, so we only run them for users on a
  // fine pointer (mouse) who haven't asked to reduce motion. Touch/RTL-mobile
  // and reduced-motion users keep the real cursor.
  ...
  const calculateRotation = () => {
    ...
    const unsortedAngle =
      Math.atan(Math.abs(state.distanceY) / Math.abs(state.distanceX)) * state.degrees;
  ...
  const renderMotionBlur = () => {
    ...
    const unsortedAngle =
      Math.atan(Math.abs(distanceY) / Math.abs(distanceX)) * state.degrees;
    let angle = 0;
    ...
    return (
      <svg
        ref={cursorRef as unknown as Ref<SVGSVGElement>}
        style={{
          ...getBaseStyle(),
          width: `${size}px`,
          height: `${size}px`,
