  ...
  return (
    <QuoteSection id="bonuses" title="מתנות" intro="כמה דברים שאני מוסיף בלי חיוב נוסף.">
      <div className="grid gap-3 sm:grid-cols-2">
        {bonuses.map((b) => (
          <div key={b.id} className="flex items-start gap-3 rounded-2xl border border-primary/20 bg-primary/5 p-4">
            <span className="grid size-8 shrink-0 place-items-center rounded-lg bg-primary/15 text-primary">
              <Gift className="size-4" />
            </span>
            <div className="min-w-0">
              <p className="text-sm font-semibold text-foreground">{b.name}</p>
              {b.desc && <p className="mt-1 text-xs leading-relaxed text-muted-foreground">{b.desc}</p>}
