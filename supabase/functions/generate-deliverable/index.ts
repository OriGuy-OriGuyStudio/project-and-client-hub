...
function copyPrompt(
  ...
  return [
    ...
    personasText
      ...
      : "",
    journeyText ? "שלבי מסע הלקוח (התאם את הקופי לשלב שהעמוד משרת):\n" + journeyText : "",
    "מבנה האתר (מפת האתר) שאתה כותב לו קופי. כתוב לכל עמוד ולכל סקשן:\n" + sitemapPagesText(sitemap),
    "כתוב תוכן מלא ואמיתי, לא רק כותרות. לכל סקשן מלא את השדות הרלוונטיים לו:",
    "- heading: כותרת ממוקדת-תועלת (לא כללית). נוסחאות לדוגמה: '{תוצאה} בלי {כאב}', 'ה{קטגוריה} ל{קהל}', שאלה שנוגעת בכאב המרכזי.",
    "- subheading: משפט שמרחיב את הכותרת ומוסיף ספציפיות.",
    "- body: פסקת תוכן אמיתית וכתובה היטב (2 עד 4 משפטים) שנוגעת בכאב או ברצון של הקהל ומחברת לתוצאה. כל סקשן תוכן (אודות, יתרונות, שירות, תהליך, סיפור, שאלות נפוצות) חייב body ממשי, לא רק כותרת.",
    "- cta: טקסט כפתור חזק (פועל + מה מקבלים), למשל 'לקביעת שיחת ייעוץ', 'לקבלת הצעת מחיר', 'להתחלה'. כל סקשן עם הנעה לפעולה (הירו, אזור הנעה לפעולה, צור קשר) חייב cta. אסור 'שלח', 'הרשמה', 'קרא עוד', 'לחץ כאן'.",
    "דוגמאות מבנה: הירו = heading חזק + subheading + cta; אזור יתרונות = heading + body שמסביר את התועלת בפועל; אודות = heading + body מספר; תהליך עבודה = heading + body; אזור הנעה לפעולה = heading + cta.",
    "חשוב מאוד: כל שדה חייב להופיע. body חייב להיות מלא בכל סקשן תוכן (לא ריק). כל סקשן שמזמין פעולה (הירו, אזור הנעה לפעולה, צור קשר, Trust Factors) חייב cta ממשי. השאר שדה כמחרוזת ריקה רק אם הוא באמת לא מתאים לסקשן (למשל cta בפוטר או subheading שמיותר).",
...
const COPY_SECTION_SCHEMA = {
  ...
  // All required so Gemini can't silently drop body/cta (optional fields get
  // skipped). The prompt says to leave a field an empty string only when it truly
  // doesn't fit the section.
  required: ["name", "heading", "subheading", "body", "cta"],
};
