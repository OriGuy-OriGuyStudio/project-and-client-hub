...
function quoteProposePrompt(payload: any): string {
  ...
  return [
    ...
    type === "website"
      ? "חשוב: הצע גם עמודים (page) וגם פיצ'רים (feature), לא רק אחד מהם. המשתמש בדרך כלל מפרט בסיכום רשימת עמודים שהוא רוצה (למשל תחת כותרת כמו 'דפים' או 'עמודים'), חובה לכלול כל עמוד כזה כפריט page נפרד, בנוסף לפיצ'רים שעולים מהצרכים והפערים."
      : "אם המשתמש ציין במפורש פריטים שהוא רוצה, כלול אותם בהצעה.",
    existing.length
      ? "פריטים שכבר זמינים (נמצאים בהיקף ההצעה או בקטלוג הקיים), אל תציע כפילויות של אלה, הצע רק פריטים חדשים שמעבר לרשימה הזו:\n" + existing.map((l) => "- " + l).join(NL)
      : "עדיין אין פריטים, אז יש הרבה מה להציע.",
    payload?.client_business ? "העסק של הלקוח: " + String(payload.client_business) : "",
