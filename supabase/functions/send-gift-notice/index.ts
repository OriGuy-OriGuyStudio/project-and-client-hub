// send-gift-notice — emails a client/partner that the admin granted them gift or
// compensation coins, waiting in the portal.
//
// Called from the admin app via supabase.functions.invoke('send-gift-notice',
// { body: { userId, kind, amount, reason } }) with the admin's JWT. verify_jwt
// stays ON; inside we additionally confirm the caller is an admin.
//
// Sends through the Gmail API as the studio account (shared OAuth secrets with
// send-invite / warranty-reminder). Best-effort: returns { ok, error }.
  ...
  <div dir="rtl" style="background:#0b0a10;padding:24px 16px;font-family:Tahoma,"Segoe UI",Arial,Helvetica,sans-serif">
    <div style="max-width:560px;margin:0 auto;background:#16151c;border:1px solid #2a2a33;border-radius:18px;overflow:hidden">
      ...
      <div dir="rtl" style="padding:28px;text-align:right;color:#e8e8ea;font-size:15px;line-height:1.7">
        <p style="margin:0 0 14px;font-size:20px;font-weight:800;color:#ffffff">${escapeHtml(opts.title)}</p>
        ${paragraphs}
...
Deno.serve(async (req) => {
  ...
  let userId = "", kind = "", amount = 0, reason = "";
  try {
  ...
  if (!userId || (kind !== "gift" && kind !== "compensation") || !amount) {
  ...
  const subject = isGift ? "🎁 קיבלת מתנה ב-Orion" : "💛 פיצוי במטבעות ב-Orion";
  const title = isGift ? "🎁 קיבלת מתנה!" : "💛 קיבלת פיצוי במטבעות";
  const lines = [
    hi,
    isGift
      ? `אורי שלח לך ${amount} מטבעות מתנה.`
      : `זוכית ב-${amount} מטבעות כפיצוי.`,
    reason ? `סיבה: ${reason}` : "",
    "המטבעות כבר ממתינות לך בממשק, ואפשר לממש אותן בחנות.",
  ].filter(Boolean);

  const html = buildHtml({ title, lines, loginUrl, studioName });
  const logContext = { user_id: userId, kind, amount, reason };
  ...
  try {
    ...
    console.error("send-gift-notice error", String(e));
    await logEmail(admin, {
      kind: "send-gift-notice",
      to_email: prof.email,
      subject,
